...
def check():
    if not os.path.isfile("outliers_checked_stayed.txt"):
        with open("outliers_checked_stayed.txt", 'w'):
            pass

    if not os.path.isfile("outliers_checked_sorted_out.txt"):
        with open("outliers_checked_sorted_out.txt", 'w'):
            pass

    new_file_stay_read = open("outliers_checked_stayed.txt", "r+")
    new_lines_stayed = new_file_stay_read.readlines()
    new_file_stay_read.close()

    new_file_sorted_out_read = open("outliers_checked_sorted_out.txt", "r+")
    new_lines_sorted_out = new_file_sorted_out_read.readlines()
    new_file_sorted_out_read.close()

    new_file_stay = open("outliers_checked_stayed.txt", "a+")
    new_file_out = open("outliers_checked_sorted_out.txt", "a+")
    file = open("outliers_unchecked.txt", "r")
