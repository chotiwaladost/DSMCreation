            ...
            if i > 0:
                for c in range(int(m * m)):
                    ...
                    if not already_created(filename, c + 1, current_output_patch):
                        base_filename = f"cut_transformed_{filename}_{str(c)}.tif"
                        tile = os.path.join(dom_path,base_filename)# Checking if it works in Linux with this structure
...
if __name__ == '__main__':
    #parent_directory = os.path.dirname(dom_path)
    #dom_path = os.path.join(parent_directory, "cut_tiles")
    #print(dom_path)
    createMatching()
