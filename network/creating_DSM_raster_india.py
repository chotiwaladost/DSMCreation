...
def test(amount, model_path, test_data_path, path_original_raster, raster_folder):
    ...
    for (data, target, src_path) in loop:
        ...
        #filename = src_path[0]
        filename = os.path.basename(src_path[0])
        # 1. Separate the name from the extension
        name_part, _ = os.path.splitext(filename)  # This gives you name_part = "cut_transformed_1938_dummy_raster_0"

        # 2. Construct the new filename with the correct extension
        #new_filename = name_part + ".tif"

          # e.g. 'ndom50_32344_5675_1_nw_2023_3~SENTINEL2X_20230515-000000-000_L3A_T32ULB_C_V1-2.npz'
        #base = filename.split('~')[0]
        #parts = base.split('_')
        #parts[-1] = str(int(parts[-1]) - 1)
        #parts[-1] = str(int(filename_part) - 1)
        #modified_base = '_'.join(parts)
        #modified_base=os.path.basename(filename)
        raster_name = f"{name_part}.tif"
...
if __name__ == '__main__':
    ...
    #r"C:\Users\renec\OneDrive\Documents\SEForALL\Local\OBI Tool\V1_DSM_creation\network\results_L1Loss_Adam_UNET_FANNED_v1\model_epoch15.pt",
     #r"C:\Users\renec\OneDrive\Documents\SEForALL\Local\OBI Tool\V1_DSM_creation\SENOI\npz_files_SEONI",
     #r"C:\Users\renec\OneDrive\Documents\SEForALL\Local\OBI Tool\V1_DSM_creation\SENOI\DummyRaster",
     #r"C:\Users\renec\OneDrive\Documents\SEForALL\Local\OBI Tool\V1_DSM_creation\SENOI\Inferred_saomi"
    #)

    test(0,
    "/home/julian/scripts/DSM_tiles_complete/results_L1Loss_Adam_UNET_FANNED_v1/model_epoch26.pt",
    "/data/inference_test/data_google/npz_googleV2",
    "/data/inference_test/data_google/Dummy_nDSM",
    "/data/inference_test/data_google/inferred_nDSMV2"
    )
