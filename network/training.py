...
def train(epoch, loader, loss_fn, optimizer, scaler, model, mse):
    ...
    for batch_index, (data, target, dataframepath) in enumerate(loop):
        if data is None:  # Check if the whole batch was bad
            print("WARNING: Skipping a batch because all samples in it were corrupt.")
            continue
        ...
        running_mse.append(mse(data.reshape(-1), target.reshape(-1)).item())
...
def run(num_epochs, lr, epoch_to_start_from):
    ...
    loss_fn = nn.L1Loss()
    #scaler = torch.cuda.amp.GradScaler() #Old
